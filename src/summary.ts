...
function displayLabel(item: ResultItem, optionIndex: number | undefined): string {
...
function itemHtml(item: ResultItem): string {
  ...
  return `<article class="item ${item.status}">${passage}<p class="status"><strong>${item.status === "wrong" ? "Wrong" : "Skipped"}</strong></p><h3>${escapeHtml(item.text)}</h3><p><strong>Your answer:</strong> ${selected}</p><p><strong>Correct answer:</strong> ${correct}</p><p><strong>Explanation:</strong> ${escapeHtml(item.explanation)}</p></article>`;
}
...
export function summaryFilename(result: TestResult): string {
  return `sai-performance-analyst-summary-${filenameTimestamp(result.submittedAt)}.html`;
}
...
export function downloadSummary(result: TestResult): void {
  ...
  link.href = url;
  link.download = summaryFilename(result);
  link.click();
