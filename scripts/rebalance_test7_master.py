...
new_data_7 = []
for q in DATA_TEST_7:
    qid = q["id"]
    if qid in fixes_test7:
        q_copy = dict(q)
        q_copy["options"] = fixes_test7[qid]
        q_copy["correct"] = 0
        new_data_7.append(q_copy)
    else:
        new_data_7.append(q)
...
print(f"Test 7 Overhauled Stats: Avg Ratio = {sum(ratios)/len(ratios):.2f}, Worst Ratio = {max(ratios):.2f}, Max Diff = {max(diffs)}")
...
with open("data_test7.py", "w") as f:
    ...
    f.write("DATA_TEST_7 = " + json.dumps(new_data_7, indent=4) + "\n")
print("Saved data_test7.py")
