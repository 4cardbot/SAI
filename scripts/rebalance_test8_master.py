...
new_data_8 = []
for q in DATA_TEST_8:
    qid = q["id"]
    if qid in fixes_test8:
        q_copy = dict(q)
        q_copy["options"] = fixes_test8[qid]
        q_copy["correct"] = 0
        new_data_8.append(q_copy)
    else:
        new_data_8.append(q)
...
print(f"Test 8 Overhauled Stats: Avg Ratio = {sum(ratios)/len(ratios):.2f}, Worst Ratio = {max(ratios):.2f}, Max Diff = {max(diffs)}")

with open("data_test8.py", "w") as f:
    f.write("# Comprehensive Master Question Bank - Mock Test 8 (100 Questions)\n")
    f.write("# Balanced Distractors, Symmetrical Lengths, No Simplistic Foils, Standard Book Citations\n\n")
    f.write("DATA_TEST_8 = " + json.dumps(new_data_8, indent=4) + "\n")
print("Saved data_test8.py")
