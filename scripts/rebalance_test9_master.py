...
new_data_9 = []
for q in DATA_TEST_9:
    qid = q["id"]
    if qid in fixes_test9:
        q_copy = dict(q)
        q_copy["options"] = fixes_test9[qid]
        q_copy["correct"] = 0
        new_data_9.append(q_copy)
    else:
        new_data_9.append(q)
...
print(f"Test 9 Overhauled Stats: Avg Ratio = {sum(ratios)/len(ratios):.2f}, Worst Ratio = {max(ratios):.2f}, Max Diff = {max(diffs)}")
...
with open("data_test9.py", "w") as f:
    ...
    f.write("DATA_TEST_9 = " + json.dumps(new_data_9, indent=4) + "\n")
print("Saved data_test9.py")
